...
public final class LocationDataStore: NSObject, ObservableObject {
    ...
    public func refreshCurrentLocationIfAuthorized() {
    ...
    private func updateCurrentLocation(to newValue: Location) {
        let oldValue = currentLocation
    ...
    private func updateCurrentLocationName() {
        ...
        Task {
            do {
                try await updateCurrentLocationName(for: currentLocation)
            } catch {
    ...
    private func shouldUpdateLocationName(oldValue: Location?, newValue: Location) -> Bool {
        guard let oldValue else {
            return true
        }
        
        let oneMinute = 60.0
        let oneKilometer = 1_000.0
        
        let nameUpdatedMoreThanOneMinuteAgo = Date.now.timeIntervalSince(currentLocationNameLastUpdated) > oneMinute
        let locationHasChangedSignificantly = newValue.toCLLocation().distance(from: oldValue.toCLLocation()) > oneKilometer
        
        return locationHasChangedSignificantly && nameUpdatedMoreThanOneMinuteAgo
    }
    
    private func updateCurrentLocationName(for newLocation: Location) async throws {
        guard let placemark = try await geocoder.reverseGeocodeLocation(newLocation.toCLLocation()).first else {
            return
        }
        
        currentLocationName = [placemark.thoroughfare, placemark.subLocality].compactMap { $0 }.joined(separator: ", ")
    ...
    private func updateNearbyStations() {
        ...
        nearbyStations = stations.allStations.nearestStations(to: currentLocation)
    }
...
extension Sequence where Element == Station {
    func nearestStations(to location: Location) -> [NearbyStation] {
