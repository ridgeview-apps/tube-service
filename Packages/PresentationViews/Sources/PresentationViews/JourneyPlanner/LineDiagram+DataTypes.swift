...
extension LineDiagramItem.Style {
    ...
    static func stopPointCircle(_ trainLineID: TrainLineID, hasTrailingLine: Bool) -> Self {
        ...
        return .init(elementType: .stopPointCircle(textOverlay),
                     lineStyle: .init(lineID: trainLineID),
...
extension ModeID {
    ...
    var image: Image {
        ...
        case .dlr, .elizabethLine, .overground, .tram, .tube, .interchangeKeepSitting, .interchangeSecure:
            Image("")
        }
    }
    
    static var journeyPlannerEditableModes: [ModeID] {
        [.tube, .bus, .overground, .nationalRail, .dlr, .elizabethLine, .tram, .riverBus, .cableCar]
    }
}

extension LineDiagramItem.Style.LineStyle {
    
    init(lineID: TrainLineID) {
        switch lineID {
        case .bakerloo, .central, .circle, .district, .hammersmithAndCity, .jubilee,
            ...
            self = .init(color: lineID.backgroundColor, fill: .solid)
        case .dlr, .elizabeth, .tram, .overground:
            self = .init(color: lineID.backgroundColor, fill: .double)
        }
    }
