...
final class CalendarExtensionsTests: XCTestCase {
    ...
    func testThisOrNextWeekend_MidWeek() {
        // Given
        ...
        // Then
        XCTAssertEqual(weekendRange?.start, saturday21st)
        XCTAssertEqual(weekendRange?.end, monday23rd) // N.B. Weekend date intervals end on Monday midnight (start of day)
    }
    
    func testThisOrNextWeekend_JustBeforeTheWeekend() {
        // Given
        let timeZone = TimeZone.london
        var calendar = Calendar(identifier: .iso8601)
        calendar.timeZone = timeZone
    ...
    func testThisOrNextWeekend_DuringTheWeekend_OnSaturday() {
        // Given
        let timeZone = TimeZone.london
        var calendar = Calendar(identifier: .iso8601)
        calendar.timeZone = timeZone
    ...
    func testThisOrNextWeekend_DuringTheWeekend_OnSunday() {
        // Given
        let timeZone = TimeZone.london
        var calendar = Calendar(identifier: .iso8601)
        calendar.timeZone = timeZone
